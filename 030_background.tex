...
Матрицей ошибок (confusion matrix) классификатора $f_\theta$ на выборке
$\{(x_i, y_i)\}_{i=1}^{n}$ называется набор из четырёх величин:
\begin{align}
...
На основании матрицы ошибок введём понятия ошибок первого и второго рода,
интерпретация которых определяется выбором <<положительного>> класса.
В нашей задаче положительным классом является \texttt{GEO} ($y = 1$).

\begin{definition}[Ошибка I рода]
\label{def:type-i-error}
Ошибкой первого рода (<<ложная тревога>>) называется условная вероятность
ошибочного отнесения объекта класса \texttt{TAG} к классу \texttt{GEO}:
\begin{equation}
    \alpha = P\bigl(f_\theta(x) = 1 \mid y = 0\bigr)
            = \frac{\mathrm{FP}}{\mathrm{FP} + \mathrm{TN}}.
    \label{eq:type-i}
\end{equation}
\end{definition}

\begin{definition}[Ошибка II рода]
\label{def:type-ii-error}
Ошибкой второго рода (<<пропуск цели>>) называется условная вероятность
ошибочного отнесения объекта класса \texttt{GEO} к классу \texttt{TAG}:
\begin{equation}
    \beta = P\bigl(f_\theta(x) = 0 \mid y = 1\bigr)
           = \frac{\mathrm{FN}}{\mathrm{FN} + \mathrm{TP}}.
    \label{eq:type-ii}
\end{equation}
\end{definition}

Заметим, что $1 - \beta$ есть мощность (power) критерия, совпадающая в
терминологии классификации с полнотой (recall) для положительного класса.
...
Поправка Бонферрони~\cite{bonferroni1936teoria} является простейшим
способом контроля FWER. При проведении $m$ одновременных проверок
...
сравнений составляет $m = \binom{k}{2} = \frac{k(k-1)}{2}$.
...
(например, $F_1$, accuracy), вычисленное на выборке $\mathcal{D}$.
...
При $n \to \infty$ эта аппроксимация является состоятельной в силу
